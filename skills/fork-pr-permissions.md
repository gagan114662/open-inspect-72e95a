...
- Round 32: **[P2]** `.github/workflows/revise-improvement-policy.yml`, existing-PR lookup: `gh pr list --head "$branch"` matches the branch name without verifying its source repository. A fork PR named `improvement-policy-proposal` can therefore be selected. The workflow then posts its regeneration comment on 
